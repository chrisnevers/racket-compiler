...
let get_var_type_test = (fun () ->
  ...
  let throws_exn () =
    let table = Hashtbl.create 1 in
    let sigma = Hashtbl.create 1 in
    let get_var_type_fun = fun () -> get_value table sigma v in
    assert_raises ~msg:"Throws error if passed a variable that is not defined"
    (TypecheckError "name not found in sigma nor gamma") get_var_type_fun
  in
  works ();
  throws_exn ();
)
...
let typecheck_exp_and_test = (fun () ->
  ...
  let works () =
    ...
    assert_equal expected (typecheck_exp exp table sigma)
  in
  let throws_exn () =
    let exp = RAnd (make_tnone (RBool true), make_tnone (RInt 0)) in
    let typecheck_exp_fun = fun () -> typecheck_exp exp table sigma in
...
let typecheck_exp_if_test = (fun () ->
  ...
  let works () =
    ...
    let actual = typecheck_exp exp table sigma in
